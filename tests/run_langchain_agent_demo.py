...
def main() -> None:
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "Summarize order ID, status, ETA, tracking number, callback ticket, hold request ID, alerts subscription ID, "
                    "and remote specialist ticket ID."
                ),
            }
        ]
    }
    ...
    if not remote_args:
        ...
        if not any(
            ...
            print(
                "FAIL: run_remote_specialist did not receive placeholderized sensitive args.",
                file=sys.stderr,
            )
            failures += 1
